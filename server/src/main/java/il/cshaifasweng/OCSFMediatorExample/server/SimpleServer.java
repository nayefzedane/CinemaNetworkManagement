...
public class SimpleServer extends AbstractServer {
	...
	private void broadcastMessageToAllClients(Object message) {
		Thread[] clientThreadList = getClientConnections();  // Get all connected clients
		for (Thread thread : clientThreadList) {
			if (thread instanceof ConnectionToClient) {
				ConnectionToClient client = (ConnectionToClient) thread;  // Correct casting
				try {
					if (client != null) {
						client.sendToClient(message);  // Send message to each client
					}
				} catch (IOException e) {
	...
	protected void handleMessageFromClient(Object msg, ConnectionToClient client) throws Exception {
		new Thread(() ->{
			...
			if (msg.equals("request_complaints_report")) {
                ...
                try {
                    complaintsList = ConnectToDatabase.getAllComplaintsOrderedByDate();
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
                try {
			...
			if (msg.equals("request_ticket_report")) {
                ...
                List<purchaseCard> purchaseList = null;
                try {
                    purchaseList = ConnectToDatabase.getAllPurchasesOrderedByDate();
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
			...
			if (msg.equals("request_purchase_link_report")) {
                ...
                List<PurchaseLink> purchaseLinkList = null;
                try {
                    purchaseLinkList = ConnectToDatabase.getAllPurchaseLinksOrderedByDate();
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
                try {
			...
			if (msg.equals("request_package_report")) {
                ...
                List<PackageCard> packageCardList = null;
                try {
                    packageCardList = ConnectToDatabase.getAllPackageCardsOrderedByDate();
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
                try {
			...
			if (msg.equals("request_price_change_requests")) {
                ...
                try {
                    requestsList = ConnectToDatabase.getAllPriceChangeRequests();
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
...
// till here **************************
			...
			if (msgString.startsWith("login@")) {
                ...
                try {
                    movies = ConnectToDatabase.getAllMovies();
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
				...
				synchronized (lock){
					...
					if (savedPurchaseCard != null) {
						try {
							...
							broadcastMessageToAllClients("request movies bro");//for testing
						} catch (Exception e) {
							e.printStackTrace();
						}
                        ...
                        try {
                            client.sendToClient("puchase card failed, the seat is already taken, sorry...");
                        } catch (IOException e) {
                            throw new RuntimeException(e);
                        }
                    }
